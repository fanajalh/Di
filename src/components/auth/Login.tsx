...
interface LoginProps {
  onNavigate: (view: 'register' | 'forgot-password' | 'landing') => void;
  onSuccess: () => void;
}
...
export default function Login({ onNavigate, onSuccess }: LoginProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setError('');
    try {
      const success = await signIn(email, password, 'credentials');
      if (success) {
        onSuccess();
      } else {
      ...
      <div className="hidden md:flex md:w-1/2 flex-col justify-between p-12 lg:p-20 bg-[#141414] border-r border-[#2A2A2A] relative overflow-hidden">
        ...
        <div className="relative z-10 max-w-lg text-left my-auto">
          <h1 className="text-4xl lg:text-6xl font-light tracking-tight leading-[1.05] mb-8 text-white uppercase">
            Hunian Premium <br />
            Level <br />
            <span className="font-semibold text-[#B0B0B0]">Selanjutnya.</span>
          </h1>
          <p className="text-sm text-[#A3A3A3] leading-relaxed max-w-md font-light">
            Platform eksklusif untuk mencari dan menyewa kost premium modular dengan visualisasi modern, sistem survey transparan, dan pembayaran instan dalam satu dashboard.
          </p>
        </div>
            ...
            <div className="space-y-2">
              ...
              <div className="text-right">
                <button 
                  ...
                  className="text-[10px] font-mono font-bold text-[#A3A3A3] hover:text-white transition-colors uppercase tracking-wider cursor-pointer"
                >
                  Lupa Password?
                </button>
              </div>
